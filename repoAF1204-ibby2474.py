...
### This program allows users to create a cookie consent interface for their web app utilising Marimo. 
#In the first cell, a UI component called a dropdown that includes the choices "Accept All Cookies" and "Reject Non-Essential," 
#which the user's cookie choice will be saved in under the variable name cookie_choice. 
#In the second cell, the user's selection is checked, rendering different components depending on the response, namely, a callout success if the former option was selected and setting cookie_state as True; a callout warning if the latter is selected and setting cookie_state as False, it just renders the dropdown.
...
#This script will produce synthetic financial information for 60 firms across 8 sectors spanning four years (2021-2024). These financial figures comprise randomised z-scores (bankruptcy risk scores), average cost of debt, and their respective lags. This will be supplemented by classifying firms into different zones of risk: distress, grey, or safe, using threshold measures from the Altman Z-score. The result is an extensively cleaned database (df_fin), excluding records that contain missing values or debt cost values exceeding 30%.
@app.cell
def _(np, pd):
    np.random.seed(7)
    SECTORS = [
...
def _(go, mo, np):
    ...
    # ---- Load assumptions from the KO Excel model -------------------------
    XLSX_URL = (
        "https://github.com/BayesUG-AI/repoAF1204-ibby2474/raw/main/"
        "KO_Acquisition_Model.xlsx"
    )
    # Assumptions sheet is a sparse label/value grid - read as raw, index by cell.
    _asm = pd.read_excel(XLSX_URL, sheet_name="Assumptions", header=None)

    def _cell(coord):
        """Return value at Excel coordinate like 'B55' from the raw grid."""
        letters = "".join(ch for ch in coord if ch.isalpha())
        row_num = int("".join(ch for ch in coord if ch.isdigit()))
        col_idx = 0
        for ch in letters:
            col_idx = col_idx * 26 + (ord(ch.upper()) - ord("A") + 1)
        return _asm.iat[row_num - 1, col_idx - 1]

    def _strip_x(v):  # exit multiples are stored like '18.0x'
        return float(str(v).rstrip("xX"))

    # Anchors (latest FY = 2024, column F)
    REV_2024   = float(_cell("F3"))     # Revenue
    CASH       = float(_cell("F60"))    # Cash & Equivalents
    TOTAL_DEBT = float(_cell("F61"))    # Total Debt
    DA_PCT     = float(_cell("B83"))    # D&A % of revenue (base case)
    TAX_RATE   = float(_cell("B84"))    # Tax rate (base case)
    SHARES_OUT = 4.3                    # Diluted shares (DCF tab B43)

    # Monte Carlo distribution parameters (rows 102-108)
    MC_REV_G1_MEAN,  MC_REV_G1_SD  = float(_cell("C102")), float(_cell("D102"))
    MC_REV_G25_MEAN, MC_REV_G25_SD = float(_cell("C103")), float(_cell("D103"))
    MC_EBITDA_MEAN,  MC_EBITDA_SD  = float(_cell("C104")), float(_cell("D104"))
    MC_WACC_MEAN,    MC_WACC_SD    = float(_cell("C106")), float(_cell("D106"))
    MC_DSO_MEAN,     MC_DSO_SD     = float(_cell("C107")), float(_cell("D107"))
    MC_CAPEX_MEAN,   MC_CAPEX_SD   = float(_cell("C108")), float(_cell("D108"))
    MC_EXIT_MIN  = _strip_x(_cell("E105"))
    MC_EXIT_MODE = _strip_x(_cell("G105"))
    MC_EXIT_MAX  = _strip_x(_cell("F105"))

    # ---- Monte Carlo simulation -------------------------------------------
    np.random.seed(42)
    N_ITER = 10_000
    rev_g1   = np.clip(np.random.normal(MC_REV_G1_MEAN,  MC_REV_G1_SD,  N_ITER),      0.01, 0.08)
    rev_g25  = np.clip(np.random.normal(MC_REV_G25_MEAN, MC_REV_G25_SD, (N_ITER, 4)), 0.01, 0.08)
    ebitda_m = np.clip(np.random.normal(MC_EBITDA_MEAN,  MC_EBITDA_SD,  N_ITER),      0.30, 0.40)
    exit_m   = np.random.triangular(MC_EXIT_MIN, MC_EXIT_MODE, MC_EXIT_MAX, N_ITER)
    wacc     = np.clip(np.random.normal(MC_WACC_MEAN,    MC_WACC_SD,    N_ITER),      0.05, 0.075)
    capex_p  = np.clip(np.random.normal(MC_CAPEX_MEAN,   MC_CAPEX_SD,   N_ITER),      0.03, 0.06)
    dso      = np.clip(np.random.normal(MC_DSO_MEAN,     MC_DSO_SD,     N_ITER),      25,   40)
    ...
    # Base case equity value/share from DCF tab 
    ko_base_case = 68.19
    ...
    for _v, _c, _lbl, _d in [
        ...
        (ko_base_case, "#d35400", f"Base Case ${ko_base_case:.2f}", "dashdot"),
    ]:
    ...
    fig_ko.update_layout(
        title="KO Equity Value Per Share - 10,000 Monte Carlo Iterations (inputs from Excel model)",
    ...
    ko_summary = mo.md(f"""
**Model:** Three-statement financial model loaded from `KO_Acquisition_Model.xlsx`
(Revenue ${REV_2024:.1f}B · Cash ${CASH:.1f}B · Debt ${TOTAL_DEBT:.1f}B · {SHARES_OUT:.1f}B shares),
followed by a 5-year DCF with EV/EBITDA exit multiple terminal value.
All seven Monte Carlo distribution parameters sourced from the Assumptions tab
(rows 102-108), simultaneously sampled across **10,000 iterations**.
...
def _(Counter, mo, pd, px, re, ui_nlp_min, ui_nlp_text):
    ...
    if len(_dfu) > 0:
        _fu = px.bar(_dfu, x="Frequency", y="Word", orientation="h", title="Top Unigrams",
            color="Frequency",
            color_continuous_scale=[[0, "#6baed6"], [0.5, "#3182bd"], [1.0, "#08519c"]],
    ...
    if len(_dfb) > 0:
        _fb = px.bar(_dfb, x="Frequency", y="Bigram", orientation="h", title="Top Bigrams",
            color="Frequency",
            color_continuous_scale=[[0, "#66c2a4"], [0.5, "#2ca25f"], [1.0, "#006d2c"]],
...
# Depends on cookie_state — renders nothing until the user makes a choice.
...
* **Investment Banking Shadow**, Observatory Capital Management LLP *(Jul – Sep 2022)* — Observed fixed-income research process at a £500M+ AUM fund
        ...
        tab_projects = mo.vstack([
            mo.md("## Interactive Features"),
            mo.md("### Altman Z-Score Calculator"),
            mo.callout(mo.md(
            ...
            z_display,
            mo.md("---"),
            mo.md("### Credit Risk Regression Analysis"),
            mo.callout(mo.md(
                "OLS regression of average cost of debt on Altman Z-Score. "
            ...
            crosstab_display,
            mo.md("---"),
            mo.md("### NLP Word & Bigram Frequency Analyser"),
            mo.callout(mo.md(
...
Piano · First-team football · County Cup cricket · Regional chess · Cycling
        ...
        _tabs = mo.ui.tabs({
            "About":              tab_about,
            "Interactive Features":  tab_projects,
